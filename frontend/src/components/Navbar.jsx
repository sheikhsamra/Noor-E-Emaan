  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-2xl border-b border-[#E8DDD1]/60 shadow-[0_4px_24px_rgba(63,49,43,0.08)]">
        <div className="container-custom flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="md:hidden h-11 w-11 rounded-2xl flex items-center justify-center bg-white border-2 border-[#E8DDD1] text-[#8A5A44] shadow-md hover:bg-[#8A5A44] hover:text-white hover:border-[#8A5A44] transition-all duration-300"
            >
              {mobileOpen
        ...
        {/* Mobile dropdown */}
        {mobileOpen && (
          <div className="md:hidden bg-white/98 backdrop-blur-xl border-t border-[#E8DDD1] px-5 py-4 flex flex-col gap-1 shadow-lg">
            {navLinks.map((item) => (
              <Link
                key={item.path}
                to={item.path}
            ...
            {user && (
              <>
                ...
                <Link to="/my-orders" onClick={() => setMobileOpen(false)}
                  className="px-4 py-3 rounded-xl text-[#3F312B] hover:bg-[#F7F2EC] hover:text-[#8A5A44] font-bold text-sm transition-all flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#8A5A44] flex-shrink-0" />
                  My Orders
                </Link>
                <button onClick={handleLogout}
                  className="px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 font-bold text-sm transition-all flex items-center gap-2 w-full text-left">
